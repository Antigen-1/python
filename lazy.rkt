#lang racket/base
(require "init.rkt" "func.rkt" "type.rkt" "value.rkt" "thread.rkt" racket/promise (for-syntax racket/base))
(provide lazy-load)
  ...
  (syntax-case stx ()
    ((_ body (arg ...) ((key value) ...) result)
     (with-syntax (((args ...) (map (lambda (s) (datum->syntax #'stx s)) (generate-temporaries #'(arg ...))))
                   ((kw ...) (map (lambda (s) (datum->syntax #'stx (string->keyword (syntax->datum s)))) (syntax->list #'(key ...))))
       ...
       #'(let ((promise (delay (let ((p body))
                                 ...
                                 p))))
           (lambda (args ... (~@ kw kwargs) ...)
             (run-and-clear
              (lambda ()
                (call-python-function #:in (list (pytuple arg ...) (pydict (list _pyunicode value) ...)) #:out result
                                      (force promise)
                                      (list args ...)
