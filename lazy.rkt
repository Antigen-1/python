...
(define-syntax (lazy-load stx)
  (syntax-case stx ()
    ((_ body (arg ...) ((key value) ...) result)
     (with-syntax (((args ...) (map (lambda (s) (datum->syntax #'stx s)) (syntax->list (generate-temporaries #'(arg ...)))))
                   ((kwargs ...) (map (lambda (s) (datum->syntax #'stx s)) (syntax->list (generate-temporaries #'(key ...))))))
